...
class SimulationServer:
    ...
    def count_reachable_floors(self):
        """Calculates the total number of floor tiles accessible from the start position."""
        if not self.current_map:
            return 0

        start_x, start_y = self.current_map.get("start", [0, 0])
        width = self.current_map["width"]
        height = self.current_map["height"]
        grid = self.current_map["grid"]

        # If the start position is somehow an obstacle, nothing is reachable
        if grid[start_y][start_x] == "obstacle":
            return 0
        ...
        while queue:
            ...
            # Check all 4 adjacent neighbors (N, S, E, W)
            neighbors = [(0, -1), (0, 1), (1, 0), (-1, 0)]
            for dx, dy in neighbors:
                nx, ny = cx + dx, cy + dy

                # If within bounds, not an obstacle, and not yet visited
                if 0 <= nx < width and 0 <= ny < height:
    ...
    def check_objective(self):
        if self.current_map["type"] == "maze":
            ...
            # FIXED: Only require the agent to visit physically reachable tiles
            reachable_floor = self.count_reachable_floors()

            if len(self.sim_state["visits"]) >= reachable_floor:
                self.running = False
                logging.info(f"Objective Reached: Room fully explored! ({reachable_floor} tiles swept)")
    ...
    def get_valid_actions(self):
        ...
        if y > 0 and self.current_map["grid"][y - 1][x] != "obstacle":
            actions.append("N")
        if y < height - 1 and self.current_map["grid"][y + 1][x] != "obstacle":
            actions.append("S")
        if x < width - 1 and self.current_map["grid"][y][x + 1] != "obstacle":
            actions.append("E")
        if x > 0 and self.current_map["grid"][y][x - 1] != "obstacle":
            actions.append("W")
        return actions
    ...
    def save_map(self, filename, map_data):
        try:
            ...
            logging.error(f"Permission denied when saving {filename}. Check Docker volume permissions.")
        except Exception as e:
